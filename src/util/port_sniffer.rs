        ...
        match TcpStream::connect_timeout(&sa, std::time::Duration::new(0, timeout)) {
            ...
            Err(e) => match e.kind() {
                io::ErrorKind::TimedOut => {
                    ...
                    tx.send(std::format!("port {} timed out.", port + i))
                        .unwrap();
                }
                _ => {
                    if verbosity >= 1 {
...
pub fn run(threads: u16, address: IpAddr, timeout: u32, verbose: u8, famous: bool) {
    let (tx, rx) = channel();
    let number_of_slots = if famous {
        ((FAMOUS / threads) as f64).ceil() as u16
    } else {
        ((MAX / threads) as f64).ceil() as u16
    };
    for i in 0..threads {
        let tx = tx.clone();
        thread::spawn(move || {
    ...
    if verbose >= 1 {
        println!(
            ...
            Color::Red
                .bold()
                .paint(format!("{} ports timed out.", count).as_str())
        );
    }
}
