...
use pktparse::tcp;
use pktparse::udp;
use tls_parser::{parse_tls_encrypted, parse_tls_plaintext, TlsMessage};
...
impl Packet {
    pub fn print(&self, verbosity: u8) {
        println!(
            "{}",
            Color::Green.paint(format!("AT TIME: {}", self.timestamp).as_str())
        );
        ...
        if verbosity > 1 {
            ...
            for err in self.errors.iter() {
                ...
                println!(
                    ...
                    Color::Cyan
                        .bold()
                        .paint(format!("{}", err.to_string()).as_str())
                );
            }
            println!("\t{}", Color::Red.paint("}"));
        }
        if verbosity > 2 {
            ...
            for utf8 in self.pe.iter() {
                for chunk in utf8.chunks(50) {
                    ...
                    println!(
                        "\t\t{}",
                        Color::White.bold().paint(format!("{}", stringed).as_str())
                    );
                }
            }
            println!("\t{}", Color::Red.paint("}"));
...
fn parse_link_layer(data: &Vec<u8>) -> Packet {
    let mut packet = Packet {
        ...
        header: Vec::new(),
        pe: Vec::new(),
        not_supported_header: None,
        not_supported_protocol: None,
        errors: Vec::new(),
    };
    match ethernet::parse_vlan_ethernet_frame(data) {
        Ok((data, headers)) => {
            match headers.ethertype {
                 ...
                 * supported types:
                 */
                ethernet::EtherType::IPv4 => match parse_ipv4(data, &mut packet) {
                    Ok(_) => (),
                    Err(e) => packet.errors.push(e),
                },
                ethernet::EtherType::IPv6 => match parse_ipv6(data, &mut packet) {
                    Ok(_) => (),
                    Err(e) => packet.errors.push(e),
                },
                ethernet::EtherType::ARP => match parse_arp(data, &mut packet) {
                    Ok(_) => (),
                    Err(e) => packet.errors.push(e),
                },
                ethernet::EtherType::Other(code) => {
                    packet
                        .errors
                        .push(format!("UNKNOWN ETHERNET FRAME TYPE CODE: {}", code));
...
fn parse_transport_layer(
    ...
    match protocol {
        ip::IPProtocol::TCP => match parse_tcp(data, packet) {
            Ok(_) => (),
            Err(e) => return Err(e.to_string()),
        },
        ip::IPProtocol::UDP => match parse_udp(data, packet) {
            Ok(_) => (),
            Err(e) => return Err(e.to_string()),
        },
...
fn parse_tcp(data: &[u8], packet: &mut Packet) -> Result<(), String> {
    match tcp::parse_tcp_header(data) {
        Ok((data, headers)) => match parse_tls(data, packet) {
            Ok(_) => packet.header.push(Header::Tcp(headers)),
            Err(e) => return Err(e),
        },
        Err(e) => {
            packet.pe.push(data.to_owned());
...
fn parse_udp(data: &[u8], packet: &mut Packet) -> Result<(), String> {
    match udp::parse_udp_header(data) {
        Ok((data, headers)) => match parse_dns(data, packet) {
            Ok(_) => {
...
fn parse_tls(data: &[u8], packet: &mut Packet) -> Result<(), String> {
    match parse_tls_plaintext(data) {
        ...
        Err(_) => match parse_tls_encrypted(data) {
            Ok((_, headers)) => {
    ...
    match dns_parser::Packet::parse(data) {
        Ok(dns_packet) => {
            ...
            let answers: Vec<String> = dns_packet
                ...
                .collect();
            packet.header.push(Header::Dns(DnsPacket {
                questions: questions,
                answers: answers,
            }));
        }
...
pub fn print_raw(len: u32, timestamp: String, data: Vec<u8>, verbosity: u8) {
    ...
    println!(
        "{}",
        Color::Green.paint(format!("AT TIME: {}", timestamp).as_str())
    );
    println!("{}{}", Color::Blue.paint("PACKET: "), Color::Red.paint("{"));
    println!("\tlength: {}", len);
    if verbosity > 1 {
