...
pub fn get_args() -> Result<Arguments, String> {
    ...
    if !host.contains("was not provided") {
        ...
        match matches.value_of("max_ttl") {
            Some(max_ttl) => match max_ttl.parse::<u8>() {
                Ok(max_ttl) => {
        ...
        match matches.value_of("start_ttl") {
            Some(start_ttl) => match start_ttl.parse::<u8>() {
                Ok(start_ttl) => {
    ...
    if !hosts.contains("was not provided") {
        for host in hosts.split(" ") {
            ...
            match host.parse::<IpAddr>() {
                Ok(ip) => {
                    println!(
                        "IP<{}> added for being pinged...",
                        RGB(223, 97, 0).paint(format!("{}", ip))
                    );
                    host_list.push(ip);
                    continue;
                }
            ...
            match (&(format!("{}:0", host))[..]).to_socket_addrs() {
                Ok(sockaddrs) => {
                    for sa in sockaddrs {
                        println!(
                            "Host<{}><{}> added for being pinged...",
                            RGB(102, 255, 255).paint(format!("{:?}", host)),
                            RGB(223, 97, 0).paint(format!("{}", sa.ip()))
                        );
    ...
    Ok(Arguments {
        ...
        verbose: Some(verbosity),
        ipaddr: Some(ip),
        mode: Modes::PortSniff,
        index: None,
        famous: Some(matches.is_present("famous")),
        hosts: None,
        max_ttl: None,
        start_ttl: None,
        max_tries: None,
